...
( function module( base ){
	...
	*/
	moduleDefine( "moduloBigNumber",
		function construct( ){
			var childprocess = require( "child_process" );
			var fs = require( "fs" );
			var path = require( "path" );
			var pathDirectory = process.cwd( ) + "/modulo-big-number/".replace( "/", path.sep );
			var javaClassFile = pathDirectory + "moduloBigNumber.class";
			var isJavaClassFileExist = fs.existsSync( javaClassFile );

			var moduloBigNumber = function moduloBigNumber( numbers, callback ){
				var parameters = Array.prototype.slice.apply( arguments );
				...
				var command = "java " + pathDirectory + "moduloBigNumber " + numbers.join( " " );	
				if( !isJavaClassFileExist ){
					command = "javac " + pathDirectory + "moduloBigNumber.java && " + command;
				}
			...
			base.moduloBigNumber = moduloBigNumber;
		} );
} )( base );
